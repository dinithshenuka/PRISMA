...
st.markdown("Easily perform systematic reviews using open databases and local/cloud AI.")
...
st.sidebar.markdown("1. The pipeline will automatically fetch from **OpenAlex** and **PubMed**.")
st.sidebar.markdown("2. To include **Scopus, Embase, IEEE**, or **ACM**, drop your CSV files into the `data/raw` folder.")
st.sidebar.markdown(f"**Current API Key Loaded:** {'✅ Yes' if os.environ.get('GROQ_API_KEY') else '❌ No (.env missing)'}")
...
with col1:
    project_name = st.text_input("Project Name", config.get('project_name', 'My_Review'))
    llm_model = st.text_input("LLM Model (Groq)", config.get('llm_model', 'llama-3.1-8b-instant'))

with col2:
    st.markdown("**Search Queries**")
    query_openalex = st.text_input("OpenAlex Query", config['search_queries'].get('openalex', ''))
    query_pubmed = st.text_input("PubMed Query", config['search_queries'].get('pubmed', ''))
...
with col3:
    inc_text = st.text_area("Inclusion Criteria (One per line)", "\n".join(config.get('inclusion_criteria', [])), height=150)
with col4:
    exc_text = st.text_area("Exclusion Criteria (One per line)", "\n".join(config.get('exclusion_criteria', [])), height=150)
...
if st.button("🚀 Start Screening", type="primary", use_container_width=True):
    if not os.environ.get('GROQ_API_KEY'):
        ...
        config['inclusion_criteria'] = [x.strip() for x in inc_text.split('\n') if x.strip()]
        config['exclusion_criteria'] = [x.strip() for x in exc_text.split('\n') if x.strip()]
        save_config(config)
        ...
        try:
            ...
            initial_counts = {'api': len(df_api), 'manual': len(df_manual), 'total': len(df_combined)}
            ...
            status_text.info(f"Phase 3: AI Screening {len(df_clean)} papers using {llm_model}... (This may take a minute)")
            ...
            if included_count > 0:
                ...
                st.download_button("Download Included Papers (CSV)", data=csv, file_name="included_papers.csv", mime="text/csv")
            else:
